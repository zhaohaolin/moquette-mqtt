...
class ProtocolProcessor {
	...
	void init(
	/* Map<String, ConnectionDescriptor> clientIDs, */
	SubscriptionsStore subscriptions, IStorageService storageService) {
	...
	void processConnect(ServerChannel session, ConnectMessage msg) {
		...
		if (m_clientIDs.containsKey(msg.getClientID())) {
			...
			ServerChannel oldSession = m_clientIDs.get(msg.getClientID())
					.getSession();
		...
		// Handle will flag
		if (msg.isWillFlag()) {
			AbstractMessage.QOSType willQos = AbstractMessage.QOSType.values()[msg
					.getWillQos()];
	...
	private void republishStored(String clientID) {
		...
		List<PublishEvent> publishedEvents = m_storageService
	...
	protected void processPublish(PublishEvent evt) {
		...
		if (qos == AbstractMessage.QOSType.LEAST_ONE) {
			...
			publishKey = String.format("%s%d", evt.getClientID(),
					evt.getMessageID());
		...
		if (qos == AbstractMessage.QOSType.LEAST_ONE) {
			if (publishKey == null) {
				throw new RuntimeException("Found a publish key null for QoS "
						+ qos + " for message " + evt);
			}
	 ...
	 * */
	private void publish2Subscribers(String topic, AbstractMessage.QOSType qos,
			byte[] message, boolean retain, Integer messageID) {
			...
			if (qos == AbstractMessage.QOSType.MOST_ONE) {
				...
				if (!sub.isCleanSession() && !sub.isActive()) {
					...
					if (qos == AbstractMessage.QOSType.EXACTLY_ONCE) {
						String publishKey = String.format("%s%d",
								sub.getClientId(), messageID);
						PublishEvent newPublishEvt = new PublishEvent(topic,
								qos, message, retain, sub.getClientId(),
								messageID, null);
	...
	private void sendPublish(String clientId, String topic,
			AbstractMessage.QOSType qos, byte[] message, boolean retained) {
	...
	private void sendPublish(String clientId, String topic,
		...
		try {
			...
			LOG.debug("Session for clientId " + clientId + " is "
					+ m_clientIDs.get(clientId).getSession());
	...
	private void sendPubRec(String clientID, int messageID) {
		LOG.debug(String.format(
				"sendPubRec invoked for clientID %s ad messageID %d", clientID,
				messageID));
	...
	void processDisconnect(ServerChannel session, String clientID,
			boolean cleanSession) throws InterruptedException {
		if (cleanSession) {
			// cleanup topic subscriptions
		...
		// evt.getSession().getAttribute(Constants.ATTR_CLIENTID);
	...
	void processUnsubscribe(ServerChannel session, String clientID,
			List<String> topics, int messageID) {
		...
		LOG.info(String
				.format("replying with UnsubAck to MSG ID %s", messageID));
	...
	private void subscribeSingleTopic(Subscription newSubscription,
		...
		Collection<HawtDBStorageService.StoredMessage> messages = m_storageService
