...
public class SubscriptionsStore {
	...
	private class SubscriptionTreeCollector implements
			IVisitor<List<Subscription>> {
	...
	public void init(IStorageService storageService) {
		...
		for (Subscription subscription : m_storageService
				.retrieveAllSubscriptions()) {
			LOG.debug("Re-subscribing " + subscription.getClientId()
					+ " to topic " + subscription.getTopic());
	...
	public void activate(String clientID) {
		LOG.debug("activate re-activating subscriptions for clientID "
				+ clientID);
	...
	public static boolean matchTopics(String msgTopic, String subscriptionTopic) {
		try {
			...
			List<Token> subscriptionTokens = SubscriptionsStore
					.splitTopic(subscriptionTopic);
			int i = 0;
			...
			if (s.isEmpty()) {
				if (i != 0) {
					throw new ParseException(
							"Bad format of topic, expetec topic name between separators",
							i);
				}
				res.add(Token.EMPTY);
			} else if (s.equals("#")) {
