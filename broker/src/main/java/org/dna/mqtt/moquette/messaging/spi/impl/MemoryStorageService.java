...
public class MemoryStorageService implements IStorageService {
	...
	public void storeRetained(String topic, byte[] message,
		...
		if (message.length == 0) {
			...
			m_retainedStore.put(topic, new HawtDBStorageService.StoredMessage(
					message, qos, topic));
		}
	}
	
	public Collection<HawtDBStorageService.StoredMessage> searchMatching(
		...
		for (Map.Entry<String, HawtDBStorageService.StoredMessage> entry : m_retainedStore
				.entrySet()) {
