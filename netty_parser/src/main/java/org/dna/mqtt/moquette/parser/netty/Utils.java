...
public class Utils {
	...
	static boolean checkHeaderAvailability(ByteBuf in) {
		...
		// return messageType == type ? MessageDecoderResult.OK :
		// MessageDecoderResult.NOT_OK;
		return true;
	}
	
	/**
	 * Decode the variable remaining length as defined in MQTT v3.1
	 * specification (section 2.1).
	 * 
	 * @return the decoded length or -1 if needed more data to decode the length
	 *         field.
	 */
	...
	/**
	 * Encode the value in the format defined in specification as variable
	 * length array.
	 * 
	 * @throws IllegalArgumentException if the value is not in the specification
	 *             bounds [0..268435455].
	 */
	static ByteBuf encodeRemainingLength(int value)
			throws CorruptedFrameException {
		if (value > MAX_LENGTH_LIMIT || value < 0) {
			throw new CorruptedFrameException("Value should in range 0.."
					+ MAX_LENGTH_LIMIT + " found " + value);
		}
	...
	static int numBytesToEncode(int len) {
		...
		throw new IllegalArgumentException(
				"value shoul be in the range [0..268435455]");
	}
